...
func _on_ingredient_trigger_body_entered(ingredient):
	...
	if not Game.Level.current_order or Game.Level.current_order and not Game.Level.current_order.is_running():
		return
	...
	if not Game.Level.current_order.is_required(ingredient.type) or Game.Level.current_order.get_remaining_amount(ingredient.type) == 0:
		return
	
	# Add ingredient to order
	Game.Level.current_order.add_to_order(ingredient.type)
